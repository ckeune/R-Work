...
m <-  read.delim("~/DIRM20142tr.txt", header=FALSE, dec=",",stringsAsFactors=FALSE,as.is=T, na.strings = ".")
m <- data.frame(m)
...
##
df1$xmin <- 0
df1$xmax <- 100 #  (colSums(m)  + rowSums(m)  )
n<-nrow(df1)
...
df1$sum1 <-  100    #colSums(m)
df1$sum2 <-  9 #numeric(n)
...
df2<-subset(df2, m>quantile(m,.75))
...
for(k in 1:nrow(df2)){
  ...
  #plot link
  circos.link(sector.index1=df1$region[i], point1=c(df1$sum1[i], abs(m[i, j])),
              sector.index2=df1$region[j], point2=c(df1$sum2[j], abs(m[i, j])),
              col = df1$lcol[i])
  
  #update sum1 and sum2 for use when plotting the next link
  df1$sum1[i] =  abs(m[i, j])
  df1$sum2[j] =   abs(m[i, j])
}
